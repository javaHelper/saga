...
public class OrderService {
	...
	@Autowired
	private OrderPurchaseProcessor orderPurchaseProcessor;
	@Autowired
	...
	public Mono<OrderPurchase> createOrder(Order order) {
		...
		// Raise an event reactively 
		orderPurchaseProcessor.process(orderPurchase);
		return Mono.just(savedOrderPurchase);
	}

	public Flux<OrderPurchase> getAll() {
		return Flux.defer(() -> Flux.fromIterable(orderPurchaseRepository.findAll())).subscribeOn(jdbcScheduler);
	}

	public Flux<List<OrderPurchase>> reactiveGetAll() {
		Flux<Long> interval = Flux.interval(Duration.ofMillis(2000));
		Flux<List<OrderPurchase>> orderPurchaseFlux = Flux
				.fromStream(Stream.generate(orderPurchaseRepository::findAll));
		
		return Flux.zip(interval, orderPurchaseFlux).map(Tuple2::getT2);
	}

	public Mono<OrderPurchase> getOrderById(Integer id) {
		return Mono
				.fromCallable(() -> orderPurchaseRepository.findById(id).orElseThrow(
						() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Order id: " + id + " does not exist")))
				.subscribeOn(jdbcScheduler);
	}

	private OrderPurchase getOrderPurchase(Order order) {
		Integer productId = order.getProductId();
		return new OrderPurchase().setProductId(productId).setUserId(order.getUserId())
