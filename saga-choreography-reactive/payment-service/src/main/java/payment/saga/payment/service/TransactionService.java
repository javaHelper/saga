...
public class TransactionService {
	...
	public Flux<Transaction> getAll() {
		return Flux.defer(() -> Flux.fromIterable(transactionRepository.findAll())).subscribeOn(jdbcScheduler);
	}

	public Flux<List<Transaction>> reactiveGetAll() {
		Flux<Long> interval = Flux.interval(Duration.ofMillis(2000));
		
		Flux<List<Transaction>> transactionFlux = Flux.fromStream(Stream.generate(transactionRepository::findAll));
		return Flux.zip(interval, transactionFlux).map(Tuple2::getT2);
	}

	public Mono<Transaction> getOrderById(Integer id) {
		return Mono.fromCallable(() -> transactionRepository.findById(id).orElseThrow(
				() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Transaction id: " + id + " does not exist")))
